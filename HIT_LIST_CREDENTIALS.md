...
**Open / click tracking (Python draft scripts only):** Pass **`--track-opens`** to add a multipart **HTML** part with a 1×1 pixel to **`GET /email_agent/open.gif?tid=<suggestion_id>`** (default host **`EMAIL_AGENT_TRACKING_BASE_URL`** = `https://edgar.truesight.me`). Pass **`--track-clicks`** to rewrite **`http(s):`** URLs in that HTML part through **`GET /email_agent/click?tid=…&r=…&to=…`** (recipient and destination are base64url, same pattern as newsletter). Plain-text part is unchanged. **Edgar** (Rails app **TrueSightDAO/sentiment_importer**, deployed at `edgar.truesight.me`) implements those routes and increments **Email Agent Drafts** columns **N** (**Open**) / **O** (**Click through**) via service account **`config/edgar_dapp_listener_key.json`**; **`sync_email_agent_followup.py`** ports them to **Email Agent Follow Up** when it matches a sent message to a draft. To fix **`pending_review`** rows where Gmail already shows **SENT** (no **DRAFT**), run **`scripts/reconcile_email_agent_drafts_stale_sent.py`** (sets **`status`** to **`sent`** and appends an audit note). To refresh MIME for **open** drafts with tracking, run **`scripts/regenerate_pending_email_agent_draft_tracking.py`** (after Edgar is deployed); it skips non-draft Gmail ids. Apps Script **`EmailAgentDrafts.gs`** does not inject pixels or wrapped links yet.

**Performance analysis / LLM tuning — which sheet to use:**
- **Email Agent Drafts** (columns **N**/**O**) is the **canonical source of truth** for open/click engagement because Edgar increments these values **in real time** every time a tracking pixel fires or a link is clicked.
- **Email Agent Follow Up** (columns **L**/**M**) copies the Drafts engagement values **exactly once** when it creates the Follow Up row at send-time. If the recipient opens or clicks **after** that sync run, the Follow Up row is **never updated** — its L/M values remain frozen at whatever they were at send-time (often `0`).
- Therefore, for **subject-line optimization**, **content A/B testing**, **open-rate analysis**, or any LLM-assisted tuning of outreach copy, query **Email Agent Drafts** (joined to Hit List via `store_key`/`hit_list_row`). Email Agent Follow Up is the right source for **cadence calculation** (when did we last send?) and **send-volume metrics**, but not for **engagement analytics**.
